...
- Apple HIG — Forms & Text Input:
  ...
  - Keyboard and scroll: When a TextField or TextEditor is focused, scroll so the active field stays visible above the keyboard. Use ScrollViewReader with \`.onChange(of: focusedField) { field in withAnimation { proxy.scrollTo(field, anchor: .center) } }\` and add \`.ignoresSafeArea(.keyboard, edges: .bottom)\` on the ScrollView so the keyboard doesn't compress the layout. Never let the keyboard cover the active field.
