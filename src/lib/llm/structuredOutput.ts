...
- Background design (default gradient by category): Never use a flat single-color background. Use a subtle gradient that fits the app's category: Fitness/Health: \`Color(hex: "0A0A1A")\` to \`Color(hex: "1C1C2E")\`; Weather: blue gradient (clear = deep blue, cloudy = grey-blue); Finance/Budget: \`Color(hex: "0A1628")\` to \`Color(hex: "1C1C1E")\`; Productivity/Todo: \`Color(hex: "1A0A2E")\` to \`Color(hex: "0A0A1A")\`; Food/Recipe: \`Color(hex: "1A0A00")\` to \`Color(hex: "1C1C1E")\`; Education: \`Color(hex: "0A1628")\` to \`Color(hex: "0A2818")\`; Utility: \`Color(hex: "1C1C1E")\` to \`Color(hex: "0A0A0A")\`; Default: dark navy to black. If the user specifies a color scheme, background, or theme, follow their specification—user instructions override. Apply as base layer: \`LinearGradient(gradient: Gradient(colors: [topColor, bottomColor]), startPoint: .top, endPoint: .bottom).ignoresSafeArea()\`.
...
- Currency input: Any field that accepts a monetary amount must show the currency symbol (e.g. $ for US) inside or preceding the field, use .keyboardType(.decimalPad), format the displayed value with NumberFormatter (.currency style, Locale.current), and never show a plain "0.00" placeholder without the symbol. Example for US: "$0.00" not "0.00".
...
- Apple HIG — Lists & Tables:
  ...
  - Every list must support swipe to delete: \`.swipeActions(edge: .trailing) { Button(role: .destructive) { /* delete */ } label: { Label("Delete", systemImage: "trash") } }\`
