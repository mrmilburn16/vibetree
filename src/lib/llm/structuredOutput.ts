...
- Live Activities: If the user asks for Live Activities, you MUST generate a WidgetKit extension implementation under a folder named exactly "WidgetExtension/" so the exporter can auto-create the extension target. Include at least:
  ...
  - Define the \`ActivityAttributes\` struct in a file that is part of the main app target (e.g. "LiveActivity/<Name>Attributes.swift" or a shared Models/ file), NOT inside WidgetExtension/. Files under WidgetExtension/ are compiled only for the widget; the main app cannot see types defined there. The app needs the type to start/update the activity; the widget references it in ActivityConfiguration(for:). Never put ActivityAttributes or any type the app references in WidgetExtension/.
  - For Dynamic Island: Use only the \`ActivityConfiguration\` \`dynamicIsland:\` parameter with the DynamicIsland result builder (DynamicIslandExpandedRegion, compactLeading, compactTrailing, minimal). Never use \`.containerBackground(for: .dynamicIsland)\` — that API does not exist and will not compile.
