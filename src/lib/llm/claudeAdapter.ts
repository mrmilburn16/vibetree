...
- Theme/accent color: Do NOT create a custom \`Theme\` struct with an \`accentColor\` property—it shadows the system API and causes "has no member 'accentColor'" errors. Use \`Color.accentColor\` (the built-in SwiftUI accent) or define a custom \`Color\` extension for app-specific colors. Do not use \`.accentColor\` where a ShapeStyle is required (e.g. in \`.foregroundStyle(.accentColor)\` or \`.tint(.accentColor)\`)—use \`Color.accentColor\` instead (e.g. \`.foregroundStyle(Color.accentColor)\`).
