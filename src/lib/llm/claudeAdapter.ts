...
- Never use Color.accentColor — it is not a valid static property on Color. Use .accentColor as a modifier (e.g. .foregroundStyle(.accent)) or get accent from the environment when needed. For explicit accent/tint use .tint(.blue) or a specific Color such as Color.blue, or a custom color from the asset catalog.
...
- Live Activities — cross-target type visibility: The ActivityAttributes struct (e.g. FocusTimerAttributes) must be defined in a file that is part of the main app target so both the app and the widget extension can see it. Put it in "LiveActivity/<Name>Attributes.swift" or a shared Models/ file that is not under WidgetExtension/. Files under WidgetExtension/ are compiled only for the widget target; the main app cannot see types defined there. The app needs the attributes type to start and update the activity (Activity.request, activity.update); the widget needs it for ActivityConfiguration(for:). Never define ActivityAttributes or any type referenced by the main app inside WidgetExtension/.
