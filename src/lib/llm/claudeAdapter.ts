...
- Backend/proxy (mandatory placeholders — never substitute):
  ...
  - EXTERNAL APIs — ALWAYS USE PROXY: Never call external APIs directly from Swift code. Never hardcode API keys, tokens, or secrets in generated Swift files. All supported external API calls must go through our backend proxy at base URL + /api/proxy/[service] (use __VIBETREE_API_BASE_URL__ for the base URL; it is replaced at build time). Available proxy services and their endpoints: Weather data: GET /api/proxy/weather. Plant identification: POST /api/proxy/plant-identify. Nearby places: GET /api/proxy/places — query params lat, lng (required), radius (meters, default 5000), category (one of coffee, gym, pharmacy), userId (optional but recommended — pass kUserId to enable per-user daily free tier tracking); returns nearby places from Apple MapKit with name, address, lat, lng, category, distance. AI completions: POST /api/proxy/ai. Email: POST /api/proxy/email — send an email. Body params: to (recipient email address), subject (email subject line), body (plain text content) or html (HTML content), userId (required). Sends from apps@vibetree.app. Costs 0.1 credits per email. Returns { success: true, messageId: string }. Images: GET /api/proxy/images — search for stock photos. Query params: query (search term, required), count (number of results, default 5, max 10), orientation (landscape, portrait, or squarish), userId (pass kUserId). Returns { results: [{ id, url, thumbUrl, smallUrl, regularUrl, photographer, photographerUrl, downloadUrl }] }. Images are hotlinked from Unsplash — do NOT download or cache them locally. Always show attribution near each image: "Photo by [photographer] on Unsplash" with a link to photographerUrl. When generating Swift code that needs any of these capabilities, construct URLRequest calls to the proxy endpoint above (e.g. kApiBaseURL + \"/api/proxy/weather\"). Pass any required parameters as JSON in the request body (POST) or as query parameters (GET). Set X-App-Token header to kAppToken on every proxy request. The proxy handles authentication and returns the API response. If a user requests a feature that requires an API not in this list, scaffold the network call with a TODO comment indicating the proxy endpoint is not yet available.
