...
| "Mac runner" / "iPhone" red X in Run on iPhone | On your Mac run `npm run dev:full` in the project root (or run `npm run mac-runner` in a second terminal). Add `MAC_RUNNER_TOKEN` to `.env.local` if missing. Connect iPhone via USB or same Wi‑Fi and tap Re-check. |
